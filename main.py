...
if __name__ == '__main__':
    ...
    img_gray = cv2.imread(str(current_path.joinpath("image/eiffel_tower_day.jpg")), cv2.IMREAD_GRAYSCALE)
    if img_gray is None:
    ...
    # Add noise
   # img_gray = add_gaussian_noise(img_gray, mean=0, sigma= 0.25)
    ...
    if hyst_method_auto:
        canny_edges = hyst_thresh_auto(edges, 0.7, 0.01)
    else:
        canny_edges = hyst_thresh(edges, 0.4, 0.05)
